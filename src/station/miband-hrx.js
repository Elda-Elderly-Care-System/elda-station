...
const UUID_PROPER = x => `0000${x}-0000-1000-8000-00805f9b34fb`;

const UUID_SERVICE_GENERIC_ACCESS     = UUID_PROPER('1800');
const UUID_SERVICE_GENERIC_ATTRIBUTE  = UUID_PROPER('1801');
const UUID_SERVICE_DEVICE_INFORMATION = UUID_PROPER('180a');
const UUID_SERVICE_FIRMWARE           = UUID_BASE('1530');
const UUID_SERVICE_ALERT_NOTIFICATION = UUID_PROPER('1811');
const UUID_SERVICE_IMMEDIATE_ALERT    = UUID_PROPER('1802');
const UUID_SERVICE_HEART_RATE         = null; // || 0x180d Does not exist for HRX.
const UUID_SERVICE_MIBAND_1           = UUID_PROPER('fee0');
const UUID_SERVICE_MIBAND_2           = UUID_PROPER('fee1');
const AUTH_KEY                        = process.env.MI_DUMMY_AUTH_KEY;
const DUMMY_AUTH_KEY                  = process.env.MI_AUTH_KEY;
...
class MiBand extends EventEmitter {
  ...
  static advertisementService() {
    ...
    return [ UUID_SERVICE_MIBAND_1, UUID_SERVICE_MIBAND_2 ];
    // return 0xFEE0;
  }

  static optionalServices(model) {
    let opts = [
    ...
    if (model !== "hrx") {
      opts.push(UUID_SERVICE_HEART_RATE);
    }
    return opts;
  }

  constructor(peripheral, model) {
    super();
    ...
    this.key = new Buffer.from(DUMMY_AUTH_KEY, 'hex');
    this.textDec = new TextDecoder();
    
    this.model = model ? model.toLowerCase() : "hrx"; // hrx, 2, 3, possibly 1

    if (this.model !== "hrx") {
      UUID_SERVICE_HEART_RATE = UUID_PROPER("180d");
    }
  ...
  async init() {
    ...
    console.log("miband_1 check.");

    if (UUID_SERVICE_HEART_RATE) {
    ...
    console.log("immediate_alert check.");
    ...
    console.log("device_information check.");
    ...
    console.log('startNotificationsForAuth check.');
    ...
    for (let char of UUID_SERVICE_HEART_RATE ? ['hrm_data', 'event', 'raw_data'] : ['event', 'raw_data']) {
  ...
  async getSwRevision() {
    ...
    data = this.textDec.decode(data)
    if (data.toLowerCase().startsWith('v'))
      data = data.substring(1)
    return data
  }
  ...
  _handleNotify(event) {
    ...
    } else if (UUID_SERVICE_HEART_RATE && event.target.uuid === this.char.hrm_data.uuid) {
